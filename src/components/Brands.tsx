    ...
    <section id="brands" className={`py-5 sm:py-6 md:py-7 lg:py-8 relative overflow-hidden ${
      ...
      >
        ...
        <div className="relative w-full overflow-hidden py-3">
          <div className="flex gap-12 sm:gap-16 items-center animate-[marquee_25s_linear_infinite] whitespace-nowrap min-w-full hover:[animation-play-state:paused]">
            ...
            {[...brands, ...brands].map((brand, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-center justify-center shrink-0 select-none group px-4 py-1"
                  title={brandName}
                >
                  <img
                    src={logoSrc}
                    alt={brandName}
                    loading="lazy"
                    className={`h-7 sm:h-8 md:h-9 w-auto max-w-[130px] sm:max-w-[150px] object-contain transition-all duration-300 filter grayscale opacity-45 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-105 ${
                      isLight
